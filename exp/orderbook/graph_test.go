...
func TestFindPaths(t *testing.T) {
	...
	expectedPaths = []Path{
		...
		// include the empty path where MTRQ is transferred without any
		// conversions
		{
			SourceAmount:      20,
			SourceAsset:       nativeAsset.String(),
...
func TestInterleavedPaths(t *testing.T) {
	...
	// There should be two paths: one that consumes the EUR/MTRQ offers and one
	// that goes through the USD/MTRQ liquidity pool.
	//
	// If we take up the offers, it's very efficient:
	//   64 CHF for 166 USD for 142 EUR for 100 MTRQ
	//
	// If we only go through pools, it's less-so:
	//   90 CHF for 152 USD for 100 MTRQ
	expectedPaths := []Path{{
		SourceAsset:       chfAsset.String(),
		SourceAmount:      64,
		DestinationAsset:  nativeAsset.String(),
...
func TestRepro(t *testing.T) {
	...
	// https://github.com/metriqorg/go/issues/4014
