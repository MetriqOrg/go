...
func TestConvertBids(t *testing.T) {
	usdGramPrice := 0.10
	basePrice := 0.10
	bids, err := convertBids(badAmtOrders, usdGramPrice, basePrice)
	assert.Error(t, err)
	...
	bids, err = convertBids(hOrders, usdGramPrice, basePrice)
...
func TestConvertAsks(t *testing.T) {
	usdGramPrice := 0.10
	basePrice := 0.10
	asks, err := convertAsks(badAmtOrders, usdGramPrice, basePrice)
	assert.Error(t, err)
	...
	asks, err = convertAsks(orders, usdGramPrice, basePrice)
	assert.NoError(t, err)
