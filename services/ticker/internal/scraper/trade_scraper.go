...
// retrieveTrades retrieves trades from the OrbitR API for the last timeDelta period.
...
func (c *ScraperConfig) retrieveTrades(since time.Time, limit int) (trades []hProtocol.Trade, err error) {
	r := orbitrclient.TradeRequest{Limit: 200, Order: orbitrclient.OrderDesc}
...
// streamTrades streams trades directly from orbitr and calls the handler function
// whenever a new trade appears.
func (c *ScraperConfig) streamTrades(h orbitrclient.TradeHandler, cursor string) error {
	if cursor == "" {
		cursor = "now"
	}

	r := orbitrclient.TradeRequest{
		Limit:  200,
		Cursor: cursor,
	}
