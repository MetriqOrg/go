...
import (
	...
	ticker "github.com/metriqorg/go/services/ticker/internal"
...
func init() {
	...
	cmdIngestTrades.Flags().BoolVar(
		...
		"stream",
		false,
		"Continuously stream new trades from the OrbitR Stream API as a daemon",
	)
...
var cmdIngestAssets = &cobra.Command{
	Use:   "assets",
	Short: "Refreshes the asset database with new data retrieved from OrbitR.",
...
var cmdIngestTrades = &cobra.Command{
	Use:   "trades",
	Short: "Fills the trade database with data retrieved form OrbitR.",
	Run: func(cmd *cobra.Command, args []string) {
		dbInfo, err := pq.ParseURL(DatabaseURL)
		if err != nil {
			Logger.Fatal("could not parse db-url:", err)
...
var cmdIngestOrderbooks = &cobra.Command{
	Use:   "orderbooks",
	Short: "Refreshes the orderbook stats database with new data retrieved from OrbitR.",
	Run: func(cmd *cobra.Command, args []string) {
		Logger.Info("Refreshing the asset database")
		dbInfo, err := pq.ParseURL(DatabaseURL)
		if err != nil {
